...
static void debug_dump(const char *text, FILE *stream, unsigned char *ptr, size_t size, char nohex) {
    size_t i;
    size_t c;
...
static int debug_log(CURL *handle, curl_infotype type, char *data, size_t size, void *user_p) {
    debug_log_options_t * config = (debug_log_options_t *)user_p;
    const char *text;
    ...
    debug_dump(text, config->stream, (unsigned char *)data, size, config->trace_ascii);
...
public:
    ...
    ~CRScUrl () {
        ...
        if (debug_log_options.stream) 
            fclose(debug_log_options.stream);
    ...
    RSL_METHOD_DECL(SetDebugLog) {
        VALUE * vString;
        int ret = 0;

        debug_log_options.trace_ascii = 1;
        ...
        if ((debug_log_options.stream = fopen(vString->value.string, "wt")) == NULL) {
            ReturnVal(V_BOOL, &ret);
            return 0;
        }

        if (func_curl_easy_setopt(curl, CURLOPT_DEBUGFUNCTION, debug_log)) {
            ReturnVal(V_BOOL, &ret);
            return 0;
        };

        if (func_curl_easy_setopt(curl, CURLOPT_DEBUGDATA, &debug_log_options)) {
            ReturnVal(V_BOOL, &ret);
            return 0;
        };

        func_curl_easy_setopt(curl, CURLOPT_VERBOSE, (void *)1);
...
private:
    ...
    debug_log_options_t debug_log_options = {1, NULL};
